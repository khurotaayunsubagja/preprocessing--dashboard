        ...
        with st.container(border=True):
            ...
            if duplicate_keys:
                ...
                if dup_count > 0:
                    ...
                    # URUTKAN BERDASARKAN KUNCI DUPLIKAT
                    # Ini akan membuat baris yang kembar saling berdampingan seperti di gambar
                    df_all_dups = df_all_dups.sort_values(by=duplicate_keys)
                    
                    # LOGIKA BARU: Styling warna menggunakan NumPy Array (Bebas Error Loop)
                    def highlight_groups(df):
                        # Palet warna pastel menyesuaikan dengan screenshot referensi
                        colors = np.array([
                        ...
                        # Hitung ID grup menggunakan .ngroup() dengan sort=False agar tidak merusak urutan
                        group_ids = df.groupby(duplicate_keys, sort=False).ngroup().values
                    ...
                    with col_dup2:
                        ...
                        if st.button("🔥 Hapus Data Duplikat", type="primary", use_container_width=True, key="btn_drop_dup"):
                            ...
                            # Kembalikan ke urutan awal responden menggunakan sort_index
